...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Area.left_bottom'
        db.delete_column('core_area', 'left_bottom')

        # Deleting field 'Area.right_top'
        db.delete_column('core_area', 'right_top')

        # Adding field 'Area.bounds'
        db.add_column('core_area', 'bounds',
                      self.gf('django.contrib.gis.db.models.fields.PolygonField')(null=True, blank=True),
                      keep_default=False)

        # Rename field 'Address.address_text'
        db.rename_column('core_address', 'address_text', 'street')

        db.alter_column('core_address', 'street', self.gf('django.db.models.fields.CharField')(max_length=255, null=True))

        # Deleting field 'Address.location'
        db.delete_column('core_address', 'location')


    def backwards(self, orm):
        # User chose to not deal with backwards NULL issues for 'Sadik.short_name'
        raise RuntimeError("Cannot reverse this migration. 'Address.address_text' and its values cannot be restored.")
